...
PROFILE_SIGNALS: dict[str, list[_Signal]] = {
    ...
    "cisco_ios": [
        ...
        (r"(?m)^enable secret\s+\d+\s+", 2.0, "IOS enable secret"),
        (r"(?m)^(no )?cdp run\b", 1.5, "IOS cdp run command"),
        (r"(?m)^(no )?ip source-route\b", 1.5, "IOS ip source-route"),
    ...
    "cisco_nxos": [
        ...
        (r"(?m)^password strength-check\b", 2.0, "NX-OS password strength-check"),
        (r"(?m)^logging server\s+\S+\s+\d+\s+use-vrf", 2.0, "NX-OS logging use-vrf"),
    ],
    "cisco_ftd": [
    ...
    "arista_eos": [
        ...
        (r"(?m)^\s+authentication mode keyboard-interactive\b", 2.0, "EOS SSH keyboard-interactive"),
    ...
    "hpe_aruba": [
        ...
        (r"(?m)^sntp server priority\b", 2.0, "ProCurve sntp server priority"),
        (r"(?m)^(no )?web-management (plaintext|ssl-cipher-low|enhanced-security|tls)\b", 2.0, "ProCurve web-management"),
        (r"(?m)^time (timezone|ntp-utc|summertime)\b", 1.5, "ProCurve time config"),
        # Shared
    ...
    "checkpoint": [
        ...
        (r"(?m)^set expert-password-hash\s+\S+", 4.0, "Gaia expert-password-hash (highly distinctive)"),
        (r"(?m)^set mgmt-gui-clients\b", 3.0, "Gaia mgmt-gui-clients restriction"),
        (r"(?m)^set ntp server (primary|secondary)\b", 2.0, "Gaia ntp server primary/secondary"),
    ...
    "nokia_sros": [
        ...
        (r"(?m)^\s+port \d+/\d+/\d+ \{", 2.0, "SR OS port chassis/slot/port block"),
        (r"(?m)^\s+management-access-filter\s*\{", 2.0, "SR OS management-access-filter block"),
    ...
    "huawei_vrp": [
        ...
        (r"(?m)^stelnet server enable\b", 3.0, "VRP stelnet server (Huawei SSH)"),
        (r"(?m)^ssh server version\s+\d", 2.0, "VRP ssh server version"),
        (r"(?m)^(undo\s+)?(ip source-route|lldp enable|ftp server enable|web-manager enable)\b", 2.0, "VRP undo hardening command"),
        (r"(?m)^snmp-agent sys-info\b", 2.0, "VRP snmp-agent sys-info"),
        (r"(?m)^hwtacacs-server\b", 3.0, "VRP hwtacacs-server block"),
        ...
        (r"(?m)^user-interface (console|vty)\s+\d", 3.0, "VRP user-interface block"),
        (r"(?m)^\s+set authentication password (cipher|irreversible-cipher)", 3.0, "VRP cipher password"),
        (r"(?m)^header (login|shell)\b", 2.0, "VRP login banner"),
        (r"(?m)^ntp-service (unicast-server|reliable|authentication)", 2.0, "VRP ntp-service"),
        (r"(?m)^Huawei Technologies\b", 4.0, "Huawei copyright header"),
        (r"(?m)^version\s+VRP\b", 4.0, "VRP version string"),
    ...
    "nokia_srl": [
        ...
        (r'"srl_nokia-[\w-]+:[\w-]+"', 5.0, "SRL Nokia namespace-qualified JSON key"),
        (r'"system".*?"information"', 2.0, "SRL system information JSON"),
        (r'"(srl_nokia-[\w-]+:)?network-instance"', 3.0, "SRL network-instance JSON"),
        (r'"interface".*?"subinterface"', 2.0, "SRL interface JSON"),
        (r'"(srl_nokia-[\w-]+:)?acl".*?"ipv[46]-filter"', 2.0, "SRL ACL filter JSON"),
        (r'"(srl_nokia-[\w-]+:)?routing-policy"', 1.5, "SRL routing-policy JSON"),
    ...
    # Input: combined/<edge>_combined.json produced by tools/velo_collector.py.
    # Detection always runs on the raw JSON file (before the parser flattens it),
    # so only JSON-level signals are listed here — flat dot-notation lines are
    # generated by the parser AFTER detection and must not appear in this list.
    "vmware_velocloud": [
        # Collector marker — written by velo_collector.py into every combined JSON
        (r'"_nd_toad_profile"\s*:\s*"vmware_velocloud"', 10.0, "VeloCloud nd-toad profile marker"),
        # JSON-level signals present in every combined JSON
    ...
    # -------------------------------------------------------- VMware NSX 4.x
    # Input: bundle JSON produced by tools/nsx_collector.py.
    # Detection always runs on the raw JSON file (before the parser flattens it),
    # so only JSON-level signals are listed here.
    "vmware_nsx": [
        # Collector always writes this marker
        (r'"_nd_toad_profile"\s*:\s*"vmware_nsx"', 10.0, "NSX nd-toad profile marker"),
        # JSON-level signals present in every NSX bundle
